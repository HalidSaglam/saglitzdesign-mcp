...
3. **Run the ones that apply, on the same directory.** Web project: `audit_project`, `audit_security`, `audit_generic_design`, `audit_seo_geo`, `audit_performance` — they share no rules and none subsumes another. Apple app: `audit_apple_ui`. Do **not** reach for `audit_project` there expecting Swift: its default extension list is CSS/SCSS/HTML/JSX/TSX/Vue/Svelte/Astro, so an Xcode project returns "Found no design source" with `filesRead: 0`. It earns its place on the web assets of a hybrid app, and its `extensions` argument *replaces* that list rather than adding to it — name every extension you want read, each with a leading dot.
