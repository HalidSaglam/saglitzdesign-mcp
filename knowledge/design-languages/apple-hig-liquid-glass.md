...
- **`NavigationView` is deprecated at 27.0 on every platform Apple lists it for.** All seven rows — iOS, iPadOS, Mac Catalyst, macOS, tvOS, visionOS and watchOS — carry `deprecatedAt: 27.0`. Their `introducedAt` values differ (iOS/iPadOS/Mac Catalyst/tvOS 13.0, macOS 10.15, visionOS 1.0, watchOS 7.0) and are not what makes this a deprecation. Every row also carries `deprecated: false`, so the deprecation lands *at* 27.0 rather than being in force on an older deployment target today. The page's deprecation summary names `NavigationStack` and `NavigationSplitView` as the replacements and points at the "Migrating to new navigation types" article for the conversion. *(SwiftUI › `NavigationView`, read from `metadata.platforms[].deprecatedAt`, `deprecated` and `deprecationSummary`.)* `NavigationStack` is also the container named above as adopting Liquid Glass automatically, so the migration and the redesign are the same piece of work.
