...
- An SF Symbol speaks a label **derived from the symbol**, and Apple's own example of that derivation produced the raw identifier `slider.vertical.3`. Apple does not publish the derivation rule, so treat "the identifier will be spoken" as a demonstrated possibility, not a guaranteed mechanism — the check Apple asks for is the same either way.
...
So the two HIG pages are compatible in principle. What Apple never publishes is a macOS-specific hit-region number, which is why the guidance below is this document's reading rather than a quotation.
