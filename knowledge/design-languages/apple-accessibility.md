...
5. **Truncation left in.** The other issue Apple names: "large text may become truncated, if it is unable to display on enough lines of text." *(WWDC24 › Get started with Dynamic Type)* — "Consider setting number of lines on the label to zero in order to allow the text to take as many lines as it needs and avoid truncations." The HIG sets the target: "aim to display as much useful text at the largest accessibility font size as you do at the largest standard font size."
...
- **For controls that genuinely cannot grow**, Apple's answer is the Large Content Viewer rather than a fixed size left unexplained — it exists "for controls that might not be able to scale with the rest of the content." *(same session)*
...
| `Image(systemName:)` | **SF Symbols carry automatic accessibility labels.** "The `checkmark.seal.fill` symbol is labeled 'Verified' by default." Where a symbol has no curated description, the label that speaks is **the symbol identifier itself** — Apple's worked example ends with a button announcing "`slider.vertical.3`". *(WWDC21 › SwiftUI Accessibility: Beyond the basics)* |
...
- `Image("cart.badge.plus")` speaks the **asset name**.
- `Image(systemName: "slider.vertical.3")` speaks the **symbol identifier** when that symbol has no curated description.
...
### The two figures do not agree, and Apple does not reconcile them

Both are current, and on macOS they point in opposite directions:
...
**They are not measuring the same thing.** A hit region is the area that responds to input; a control size is the area that is drawn. A hit region can legitimately be larger than its control — that is the standard fix for a small glyph button. So the two pages are compatible in principle; what Apple never publishes is a macOS-specific hit-region number.
...
| "So SwiftUI has it covered." | The automatic label is frequently a **developer identifier**, not a description: `Image("cart.badge.plus")` speaks its asset name, and a symbol with no curated description speaks its own identifier — Apple's own example ends with a button announcing "`slider.vertical.3`" despite being initialised with the title "Edit Budgets". Apple's instruction is to check: "If you're relying on a symbol's default label, it's important to check that it accurately describes your interface." |
...
| "44×44 pt is the minimum control size on every Apple platform." | Two different measurements. 44×44 pt is the general rule for a **button's hit region**, with visionOS (60×60 pt) as its only stated exception *(HIG › Buttons)*. The per-platform **control size** table gives macOS 28×28 pt default / 20×20 pt minimum, tvOS 66×66 / 56×56, visionOS 60×60 / 28×28 *(HIG › Accessibility)*. Apple publishes no macOS hit-region figure and reconciles the two nowhere — see §3. |
...
**Xcode previews are the fastest Dynamic Type check.** "In Xcode, navigate to the Preview canvas, and click the Variants button. Then, select Dynamic Type Variants. Xcode will generate a preview for every variant of the text sizes available, so you can quickly locate issues for a particular view." The Xcode debugger can also override Dynamic Type and other accessibility settings. *(WWDC24 › Get started with Dynamic Type)*
