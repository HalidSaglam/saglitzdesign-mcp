...
**Which of these numbers are Apple's.** Apple publishes the menu bar height (24 pt), the macOS body size (13 pt) and recommended minimum (10 pt), the full macOS text-style scale, and the split-view thin divider width ("The thin divider measures one point in width" — HIG › Split views, macOS). It publishes that sidebar row height and glyph size follow a small/medium/large setting people change in General settings, **without giving point values**. It publishes no unified-toolbar height, no toolbar symbol size, no sidebar or inspector width, no table row height and no minimum click target for macOS. The remaining rows above are working conventions measured from shipping Apple apps — good defaults, not specifications. **A rule must not report a deviation from an unpublished number as a HIG violation.**
...
**What Apple does not define.** These are widely used Mac conventions that Apple's standard table does not contain, checked against the HIG keyboards page, the HIG menu bar page, the SwiftUI `keyboardShortcut(_:modifiers:)` reference and the WWDC transcript corpus:

- **A sidebar toggle.** The View menu carries a "Show/Hide Sidebar" item, but Apple assigns it no shortcut. ⌃⌘S is app convention (Mail, Notes), not a published standard, and Finder uses ⌥⌘S. A rule must not assert ⌃⌘S as Apple's.
...
| `refreshable(action:)` — macOS 12+ | Pull-to-refresh has no pointer equivalent; the Mac answer is a menu command with a shortcut. Note that Apple does **not** publish ⌘R as a standard refresh shortcut (see §3) — cite the menu item, not the key. |
...
5. **"⌃⌘S is the standard sidebar shortcut."** Apple's standard-shortcut table contains no sidebar entry at all (see §3). The View menu item is standard; the key is not. Finder uses ⌥⌘S.
...
10. **"The unified toolbar is 52pt."** Apple publishes no macOS toolbar height. See the note under the metrics table for which numbers here are Apple's and which are convention.
