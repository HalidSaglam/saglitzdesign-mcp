...
| Sidebar width | 220–320pt, resizable; collapse via a View-menu command (Apple assigns no shortcut) |
...
*Published only qualitatively:* sidebar row height and glyph size follow a small/medium/large setting people change in General settings, **with no point values given**.
...
**Everything in the table above that is not listed as published in this note is a working convention** measured from shipping Apple apps — good defaults, not specifications, whether or not it is named in the paragraph above. **A rule must not report a deviation from a convention as a HIG violation.** The published figures are fair game.
...
- **A sidebar toggle.** The View menu carries a "Show/Hide Sidebar" item, and **the HIG's standard-shortcut table assigns it no key** — the ~120-row table has no sidebar entry. Apple does bind one *somewhere*: its end-user Mac keyboard shortcuts article publishes "Option-Command-S: Hide or show the Sidebar in Finder windows" (`support.apple.com/en-us/102650`, outside the developer-documentation surfaces this document's `sources:` list covers). So the accurate statement is that **⌥⌘S is Finder's, published for users; ⌃⌘S is app convention (Mail, Notes) published nowhere; and the HIG gives app developers no standard at all.** A rule must not assert ⌃⌘S as Apple's, and must not tell an app that binds ⌥⌘S that it invented the key.
...
5. **"⌃⌘S is the standard sidebar shortcut."** The HIG's standard-shortcut table contains no sidebar entry at all (see §3). The View menu item is standard; the key is not. The one key Apple does publish for it is Finder's ⌥⌘S, and only in the end-user shortcut article — so an app that binds ⌥⌘S is following Apple, and one that binds ⌃⌘S is following Mail and Notes. Neither is a HIG violation, and neither is a HIG requirement.
