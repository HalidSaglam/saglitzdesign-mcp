...
*Not published anywhere Apple documents:* the unified-toolbar height, the toolbar symbol size, sidebar and inspector widths, and table row heights. Those rows above are working conventions measured from shipping Apple apps — good defaults, not specifications. **A rule must not report a deviation from one of those as a HIG violation** — but note that this sentence covers only the unpublished figures listed in this paragraph, not the published ones above it.
