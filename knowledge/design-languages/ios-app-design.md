...
**Where Apple stops:** the tab-bars page gives **no numeric tab count**. "Use the appropriate number of tabs required to help people navigate your app… keep in mind that it's generally easier to navigate among fewer tabs." The only count on the page is for customizable iPad tab bars: "If you let people select their own tabs, **aim for a default list of five or fewer** to preserve continuity between compact and regular view sizes." (The page's other figures are tvOS geometry — "The height of a tab bar is 68 points, and its top edge is 46 points from the top of the screen" — which say nothing about iPhone.) So 2–5 is a sound working default and the *overflow* rule is the enforceable one — write a rule against the More tab appearing, not against a tab count.
...
- **Touch targets: 44×44pt.** Apple uses that figure two ways — "As a general rule, a button needs a hit region of at least 44x44 pt" *(HIG › Buttons)*, and 44×44 pt as the iOS/iPadOS **default control size**, against a published **minimum of 28×28 pt** *(HIG › Accessibility, Mobility)*. On iOS the two coincide, so 44×44 is the right number to design to; the 28×28 minimum is what a drawn control may not go below. See `apple-accessibility` §3 — and do not carry the 44 pt figure to macOS, where the default is 28×28. 60pt for primary CTAs and ≥8pt between tappables are convention; Apple's published spacing guidance is ~12 pt of padding around bezelled elements and ~24 pt around unbezelled ones.
...
9. **"The grabber is 36×5pt", "the alert is 270pt", "a popover is at least 320pt wide."** None of the three is published: Apple describes the grabber as "a small horizontal indicator"; HIG › Popovers contains no pt figure at all; HIG › Alerts contains exactly one, and it is the maximum height of a *visionOS* accessory view. See "Which of these numbers are Apple's" under the metrics table in §1 before reporting a deviation as a HIG violation.
