...
export const PROJECT_NOT_VISIBLE: string[] = [
  "**Nothing here is measured, and nothing is rendered.** No contrast ratio is computed, no tap target sized, no page loaded, no screenshot taken, no browser involved. A stylesheet declaring `.a { color: #777777; background: #888888; padding: 13px 27px; }` came back as one `hardcoded-color` warning, which says nothing whatever about how those two colours read against each other, and nothing about the spacing either. The numbers above come from the text that was read — counts of the values written there, and the colour-distance arithmetic that calls two of them indistinguishable — or, in the budget column, from constants picked in advance. None of them comes from a rendered page.",
